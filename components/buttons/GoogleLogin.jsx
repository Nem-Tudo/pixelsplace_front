...
export default function GoogleLogin({ onUpdateLoading = () => { }, customStyle = {} }) {
    ...
    const handleLogin = () => {
        ...
        const authUrl = `${settings.apiURL}/auth/google`;
    ...
    return (
        <button
            ...
            style={customStyle}
        >
            <img style={{ objectFit: "unset" }} ref={iconRef} className={styles.icon} src='/assets/google.svg' />
            <span className={styles.text} style={isError.error ? { color: "red" } : {}}>
                {
                    isError.error ? `Erro: ${isError.reason}` : (isLoading ? 'Entrando...' : 'Entrar com Google')
                }
            </span>
        </button>
    );

...
export default function Button({ label = 'Botão', href = undefined, hue = 207.04, on_click = undefined, hierarchy = 1 }) {
    ...
    if (href == undefined && on_click == undefined) { // botão não tem href nem clique
        ...
        return <Link className={styles.btn + importances[hierarchy]} data-hue={hue} href={href}>{label}</Link>
        ...
        return <Link className={styles.btn + importances[hierarchy]} data-hue={hue} href={href} onClick={() => on_click()}>{label}</Link>
    }
}

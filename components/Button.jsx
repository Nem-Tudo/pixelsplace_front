...
export default function Button({ label = 'Botão', href = undefined, hue = 207.04, on_click = undefined }) {
    if(href == undefined && on_click == undefined) { // botão não tem href nem clique
        ...
        return <button className={styles.btn} data-hue={hue} onClick={() => on_click()}>{label}</button>
    } else { // tem href e tem clique
        return <a className={styles.btn} data-hue={hue} href={href} onClick={() => on_click()}>{label}</a>
    }
}

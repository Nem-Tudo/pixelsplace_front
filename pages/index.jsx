      ...
      <MainLayout>
        <main className={styles.main}>
          ...
          <div className={styles.buttons}>
            ...
            {
              checkFlags(loggedUser?.flags, "LANGUAGE_TEST") && <>
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                  <button onClick={() => changeLanguage('en')}>English</button>
                  <button onClick={() => changeLanguage('pt')}>Português</button>
                  <p>Idioma atual: {language.getString("_LANGUAGE.NAME")}</p>
                  <p>Sim (COMMON.YES): {language.getString("COMMON.YES")}</p>
                  <p>Bandeira: {language.getString("_LANGUAGE.FLAG")}</p>
                  <p>Test: {language.getString("TEST1", { user: loggedUser?.display_name })}</p>
                </div>
              </>
            }
